...
public abstract class MoreSilkbagCard extends SilkbagCard {
    ...
    public void effect() throws DesktopException {
        for (CompletePlayer completePlayer : getPlayer()) {
            MessageReceipt.globalInContext(CardDesktop.playerInContext() +"将对"+completePlayer.getId()+"使用"+ CardDesktop.cardInContext());
            try {
...
//            log.debug("{}完成：执行玩家：{}，被执行玩家：{}",getName() , ContextManage.desktop().getPlayer(), completePlayer);
            MessageReceipt.globalInContext(CardDesktop.playerInContext() +"完成对"+completePlayer.getId()+"使用"+CardDesktop.cardInContext());
        }
    }
